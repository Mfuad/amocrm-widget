        ...
        $(".copy-lead__button").on("click", function () {
          if (!$(this).hasClass("copy-lead__button_disable")) {
            ...
            $.ajax({
              ...
              async: false,
              beforeSend: function () {
                $(".copy-lead__info").text("Загрузка...");
              },
            }).done(function (data) {
              leadData = data;
            });

            for (let key in leadData._embedded) {
              for (let item of leadData._embedded[key]) {
                delete item._links;
              }
            }
            for (let key in leadData) {
              if (leadData[key] === null) delete leadData[key];
            }
            delete leadData.id;
            delete leadData._links;
            ...
            $.ajax({
              ...
              dataType: "json",
              async: false,
              data: JSON.stringify([{ ...leadData, pipeline_id, status_id }]),
            }).done(function () {
              success = true;
            });
